...
open class UShortDeque(
  initialCapacity: Int = 16,
  val scaleFactor: Float = 1.5F,
  val maxSize: Int = 2_147_483_647,
) {

  protected var buffer = UShortArray(initialCapacity)
   ...
   */
  fun popFirstOr(value: UShort) = if (isEmpty()) value else unsafePopFirst()
   ...
   * was empty.
   */
  fun popFirstOrGet(fn: () -> UShort) = if (isEmpty()) fn() else unsafePopFirst()
  ...
  private fun unsafePopFirst(): UShort {
    val c = buffer[realHead]
   ...
   * @throws NoSuchElementException If this deque is empty.
   */
  fun peekFirst() = if (isEmpty()) throw NoSuchElementException() else buffer[realHead]
   ...
   */
  fun peekFirstOrNull() = if (isEmpty()) null else buffer[realHead]
   ...
   */
  fun peekFirstOr(value: UShort) = if (isEmpty()) value else buffer[realHead]
   ...
   */
  fun peekFirstOrGet(fn: () -> UShort) = if (isEmpty()) fn() else buffer[realHead]
   ...
   * @throws Throwable If this deque is empty.
   */
  fun peekFirstOrThrow(fn: () -> Throwable) = if (isEmpty()) throw fn() else buffer[realHead]
   ...
   * if this deque was non-empty, or [value] if this deque was empty.
   */
  fun popLastOr(value: UShort) = if (isEmpty()) value else unsafePopLast()
   ...
   * was empty.
   */
  fun popLastOrGet(fn: () -> UShort) = if (isEmpty()) fn() else unsafePopLast()
  ...
  private fun unsafePopLast(): UShort {
    val c = buffer[internalIndex(lastIndex)]
    size--
    return c
  }
   ...
   * @throws NoSuchElementException If this deque is empty.
   */
  fun peekLast() = if (isEmpty()) throw NoSuchElementException() else buffer[internalIndex(lastIndex)]
   ...
   * `null` if this deque is empty.
   */
  fun peekLastOrNull() = if (isEmpty()) null else buffer[internalIndex(lastIndex)]
   ...
   * [value] if this deque is empty.
   */
  fun peekLastOr(value: UShort) = if (isEmpty()) value else buffer[internalIndex(lastIndex)]
   ...
   * the value returned by [fn] if this deque is empty.
   */
  fun peekLastOrGet(fn: () -> UShort) = if (isEmpty()) fn() else buffer[internalIndex(lastIndex)]
   ...
   * @throws Throwable If this deque is empty.
   */
  fun peekLastOrThrow(fn: () -> Throwable) = if (isEmpty()) throw fn() else buffer[internalIndex(lastIndex)]
  ...
  inline fun prepend(value: UShort) = pushFirst(value)
   ...
   */
  @Suppress("NOTHING_TO_INLINE")
  inline fun append(value: UShort) = pushLast(value)
   ...
   * than [lastIndex].
   */
  operator fun get(index: Int) = buffer[toValidInternalIndex(index)]
  ...
  @Suppress("NOTHING_TO_INLINE")
  inline operator fun plusAssign(value: UShort) = pushLast(value)
   ...
   */
  fun popEach(reverse: Boolean = false, fn: (value: UShort) -> Unit) {
    if (reverse)
      while (isNotEmpty())
        fn(unsafePopLast())
    else
   ...
   * @param fn Function to call with each item removed from this deque.
   */
  fun popEachIndexed(reverse: Boolean = false, fn: (index: Int, value: UShort) -> Unit) {
    if (reverse) {
      var i = size
      while (isNotEmpty())
        fn(--i, unsafePopLast())
   ...
   */
  fun peekEach(reverse: Boolean = false, fn: (value: UShort) -> Unit) {
    if (reverse)
   ...
   * @param fn Function to call with each item in this deque.
   */
  fun peekEachIndexed(reverse: Boolean, fn: (index: Int, value: UShort) -> Unit) {
    if (reverse)
      for (i in lastIndex downTo 0)
        fn(i, get(i))
    else
  ...
  fun ensureCapacity(minCapacity: Int) {
    when {
      ...
      // new array.
      isEmpty()
      -> buffer = UShortArray(minCapacity)
  ...
  fun contentEquals(other: UShortDeque): Boolean {
    if (size != other.size)
      return false
    if (size == 0)
      return true
  ...
  fun copyOf(): UShortDeque {
    val out = UShortDeque(0, scaleFactor, maxSize)
  ...
  override fun equals(other: Any?) = if (other is UShortDeque) buffer.contentEquals(buffer) else false
  ...
  protected fun copyElements(newCapacity: Int) {
    val new = UShortArray(newCapacity)
