...
open class ULongDeque(
  initialCapacity: Int = 16,
  val scaleFactor: Float = 1.5F,
  val maxSize: Int = 2_147_483_647,
) {

  protected var buffer = ULongArray(initialCapacity)
   ...
   */
  fun popFirstOr(value: ULong) = if (isEmpty()) value else unsafePopFirst()
   ...
   * was empty.
   */
  fun popFirstOrGet(fn: () -> ULong) = if (isEmpty()) fn() else unsafePopFirst()
  ...
  private fun unsafePopFirst(): ULong {
   ...
   */
  fun peekFirstOr(value: ULong) = if (isEmpty()) value else buffer[realHead]
   ...
   */
  fun peekFirstOrGet(fn: () -> ULong) = if (isEmpty()) fn() else buffer[realHead]
   ...
   * if this deque was non-empty, or [value] if this deque was empty.
   */
  fun popLastOr(value: ULong) = if (isEmpty()) value else unsafePopLast()
   ...
   * was empty.
   */
  fun popLastOrGet(fn: () -> ULong) = if (isEmpty()) fn() else unsafePopLast()
  ...
  private fun unsafePopLast(): ULong {
   ...
   * [value] if this deque is empty.
   */
  fun peekLastOr(value: ULong) = if (isEmpty()) value else buffer[internalIndex(lastIndex)]
   ...
   * the value returned by [fn] if this deque is empty.
   */
  fun peekLastOrGet(fn: () -> ULong) = if (isEmpty()) fn() else buffer[internalIndex(lastIndex)]
  ...
  inline fun prepend(value: ULong) = pushFirst(value)
   ...
   */
  @Suppress("NOTHING_TO_INLINE")
  inline fun append(value: ULong) = pushLast(value)
  ...
  @Suppress("NOTHING_TO_INLINE")
  inline operator fun plusAssign(value: ULong) = pushLast(value)
   ...
   */
  fun popEach(reverse: Boolean = false, fn: (value: ULong) -> Unit) {
    if (reverse)
      while (isNotEmpty())
        fn(unsafePopLast())
    else
   ...
   */
  fun peekEach(reverse: Boolean = false, fn: (value: ULong) -> Unit) {
    if (reverse)
   ...
   * @param fn Function to call with each item in this deque.
   */
  fun peekEachIndexed(reverse: Boolean, fn: (index: Int, value: ULong) -> Unit) {
    if (reverse)
      for (i in lastIndex downTo 0)
        fn(i, get(i))
    else
  ...
  fun contentEquals(other: ULongDeque): Boolean {
    if (size != other.size)
      return false
    if (size == 0)
      return true
  ...
  fun copyOf(): ULongDeque {
    val out = ULongDeque(0, scaleFactor, maxSize)
  ...
  override fun equals(other: Any?) = if (other is ULongDeque) buffer.contentEquals(buffer) else false
  ...
  protected fun copyElements(newCapacity: Int) {
    val new = ULongArray(newCapacity)
