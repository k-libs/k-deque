...
open class UByteDeque(
  initialCapacity: Int = 16,
  val scaleFactor: Float = 1.5F,
  val maxSize: Int = 2_147_483_647,
) {

  protected var buffer = UByteArray(initialCapacity)
   ...
   */
  fun popFirstOr(value: UByte) = if (isEmpty()) value else unsafePopFirst()
   ...
   * was empty.
   */
  fun popFirstOrGet(fn: () -> UByte) = if (isEmpty()) fn() else unsafePopFirst()
  ...
  private fun unsafePopFirst(): UByte {
   ...
   */
  fun peekFirstOr(value: UByte) = if (isEmpty()) value else buffer[realHead]
   ...
   */
  fun peekFirstOrGet(fn: () -> UByte) = if (isEmpty()) fn() else buffer[realHead]
   ...
   * if this deque was non-empty, or [value] if this deque was empty.
   */
  fun popLastOr(value: UByte) = if (isEmpty()) value else unsafePopLast()
   ...
   * was empty.
   */
  fun popLastOrGet(fn: () -> UByte) = if (isEmpty()) fn() else unsafePopLast()
  ...
  private fun unsafePopLast(): UByte {
   ...
   * [value] if this deque is empty.
   */
  fun peekLastOr(value: UByte) = if (isEmpty()) value else buffer[internalIndex(lastIndex)]
   ...
   * the value returned by [fn] if this deque is empty.
   */
  fun peekLastOrGet(fn: () -> UByte) = if (isEmpty()) fn() else buffer[internalIndex(lastIndex)]
   ...
   */
  fun popEach(reverse: Boolean = false, fn: (value: UByte) -> Unit) {
    if (reverse)
      while (isNotEmpty())
        fn(unsafePopLast())
    else
   ...
   * @param fn Function to call with each item removed from this deque.
   */
  fun popEachIndexed(reverse: Boolean = false, fn: (index: Int, value: UByte) -> Unit) {
    if (reverse) {
      var i = size
      while (isNotEmpty())
        fn(--i, unsafePopLast())
   ...
   */
  fun peekEach(reverse: Boolean = false, fn: (value: UByte) -> Unit) {
    if (reverse)
   ...
   * @param fn Function to call with each item in this deque.
   */
  fun peekEachIndexed(reverse: Boolean, fn: (index: Int, value: UByte) -> Unit) {
    if (reverse)
      for (i in lastIndex downTo 0)
        fn(i, get(i))
    else
  ...
  fun copyOf(): UByteDeque {
    val out = UByteDeque(0, scaleFactor, maxSize)
  ...
  protected fun copyElements(newCapacity: Int) {
    val new = UByteArray(newCapacity)
