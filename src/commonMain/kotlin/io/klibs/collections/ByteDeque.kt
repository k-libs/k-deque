 ...
 */
open class ByteDeque(
  initialCapacity: Int = 16,
  val scaleFactor: Float = 1.5F,
  val maxSize: Int = 2_147_483_647,
) {

  protected var buffer = ByteArray(initialCapacity)
   ...
   */
  fun popFirstOr(value: Byte) = if (isEmpty()) value else unsafePopFirst()
   ...
   * was empty.
   */
  fun popFirstOrGet(fn: () -> Byte) = if (isEmpty()) fn() else unsafePopFirst()
  ...
  private fun unsafePopFirst(): Byte {
   ...
   */
  fun peekFirstOr(value: Byte) = if (isEmpty()) value else buffer[realHead]
   ...
   */
  fun peekFirstOrGet(fn: () -> Byte) = if (isEmpty()) fn() else buffer[realHead]
   ...
   * if this deque was non-empty, or [value] if this deque was empty.
   */
  fun popLastOr(value: Byte) = if (isEmpty()) value else unsafePopLast()
   ...
   * was empty.
   */
  fun popLastOrGet(fn: () -> Byte) = if (isEmpty()) fn() else unsafePopLast()
  ...
  private fun unsafePopLast(): Byte {
   ...
   * [value] if this deque is empty.
   */
  fun peekLastOr(value: Byte) = if (isEmpty()) value else buffer[internalIndex(lastIndex)]
   ...
   * the value returned by [fn] if this deque is empty.
   */
  fun peekLastOrGet(fn: () -> Byte) = if (isEmpty()) fn() else buffer[internalIndex(lastIndex)]
  ...
  inline fun prepend(value: Byte) = pushFirst(value)
   ...
   */
  @Suppress("NOTHING_TO_INLINE")
  inline fun append(value: Byte) = pushLast(value)
  ...
  @Suppress("NOTHING_TO_INLINE")
  inline operator fun plusAssign(value: Byte) = pushLast(value)
   ...
   */
  fun popEach(reverse: Boolean = false, fn: (value: Byte) -> Unit) {
    if (reverse)
      while (isNotEmpty())
        fn(unsafePopLast())
    else
   ...
   * @param fn Function to call with each item removed from this deque.
   */
  fun popEachIndexed(reverse: Boolean = false, fn: (index: Int, value: Byte) -> Unit) {
    if (reverse) {
      var i = size
      while (isNotEmpty())
        fn(--i, unsafePopLast())
   ...
   */
  fun peekEach(reverse: Boolean = false, fn: (value: Byte) -> Unit) {
    if (reverse)
   ...
   * @param fn Function to call with each item in this deque.
   */
  fun peekEachIndexed(reverse: Boolean, fn: (index: Int, value: Byte) -> Unit) {
    if (reverse)
      for (i in lastIndex downTo 0)
        fn(i, get(i))
    else
  ...
  fun contentEquals(other: ByteDeque): Boolean {
    if (size != other.size)
      return false
    if (size == 0)
      return true
