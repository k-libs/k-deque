...
open class IntDeque(
  ...
  protected var buffer = IntArray(initialCapacity)
   ...
   */
  fun popFirstOr(value: Int) = if (isEmpty()) value else unsafePopFirst()
   ...
   * was empty.
   */
  fun popFirstOrGet(fn: () -> Int) = if (isEmpty()) fn() else unsafePopFirst()
  ...
  private fun unsafePopFirst(): Int {
   ...
   */
  fun peekFirstOr(value: Int) = if (isEmpty()) value else buffer[realHead]
   ...
   */
  fun peekFirstOrGet(fn: () -> Int) = if (isEmpty()) fn() else buffer[realHead]
   ...
   * if this deque was non-empty, or [value] if this deque was empty.
   */
  fun popLastOr(value: Int) = if (isEmpty()) value else unsafePopLast()
   ...
   * was empty.
   */
  fun popLastOrGet(fn: () -> Int) = if (isEmpty()) fn() else unsafePopLast()
   ...
   * [value] if this deque is empty.
   */
  fun peekLastOr(value: Int) = if (isEmpty()) value else buffer[internalIndex(lastIndex)]
   ...
   * the value returned by [fn] if this deque is empty.
   */
  fun peekLastOrGet(fn: () -> Int) = if (isEmpty()) fn() else buffer[internalIndex(lastIndex)]
  ...
  inline fun prepend(value: Int) = pushFirst(value)
   ...
   */
  @Suppress("NOTHING_TO_INLINE")
  inline fun append(value: Int) = pushLast(value)
  ...
  @Suppress("NOTHING_TO_INLINE")
  inline operator fun plusAssign(value: Int) = pushLast(value)
   ...
   * @param fn Function to call with each item removed from this deque.
   */
  fun popEachIndexed(reverse: Boolean = false, fn: (index: Int, value: Int) -> Unit) {
    if (reverse) {
      var i = size
      while (isNotEmpty())
        fn(--i, unsafePopLast())
   ...
   * @param fn Function to call with each item in this deque.
   */
  fun peekEachIndexed(reverse: Boolean, fn: (index: Int, value: Int) -> Unit) {
    if (reverse)
      for (i in lastIndex downTo 0)
        fn(i, get(i))
    else
  ...
  fun contentEquals(other: IntDeque): Boolean {
    if (size != other.size)
      return false
    if (size == 0)
      return true
