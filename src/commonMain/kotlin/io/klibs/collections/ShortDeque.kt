...
open class ShortDeque(
  ...
  protected var buffer = ShortArray(initialCapacity)
   ...
   */
  fun popFirstOr(value: Short) = if (isEmpty()) value else unsafePopFirst()
   ...
   * was empty.
   */
  fun popFirstOrGet(fn: () -> Short) = if (isEmpty()) fn() else unsafePopFirst()
  ...
  private fun unsafePopFirst(): Short {
   ...
   */
  fun peekFirstOr(value: Short) = if (isEmpty()) value else buffer[realHead]
   ...
   */
  fun peekFirstOrGet(fn: () -> Short) = if (isEmpty()) fn() else buffer[realHead]
   ...
   * if this deque was non-empty, or [value] if this deque was empty.
   */
  fun popLastOr(value: Short) = if (isEmpty()) value else unsafePopLast()
   ...
   * was empty.
   */
  fun popLastOrGet(fn: () -> Short) = if (isEmpty()) fn() else unsafePopLast()
  ...
  private fun unsafePopLast(): Short {
   ...
   * [value] if this deque is empty.
   */
  fun peekLastOr(value: Short) = if (isEmpty()) value else buffer[internalIndex(lastIndex)]
   ...
   * the value returned by [fn] if this deque is empty.
   */
  fun peekLastOrGet(fn: () -> Short) = if (isEmpty()) fn() else buffer[internalIndex(lastIndex)]
  ...
  inline fun prepend(value: Short) = pushFirst(value)
   ...
   */
  @Suppress("NOTHING_TO_INLINE")
  inline fun append(value: Short) = pushLast(value)
  ...
  @Suppress("NOTHING_TO_INLINE")
  inline operator fun plusAssign(value: Short) = pushLast(value)
   ...
   */
  fun popEach(reverse: Boolean = false, fn: (value: Short) -> Unit) {
    if (reverse)
      while (isNotEmpty())
        fn(unsafePopLast())
    else
   ...
   * @param fn Function to call with each item removed from this deque.
   */
  fun popEachIndexed(reverse: Boolean = false, fn: (index: Int, value: Short) -> Unit) {
    if (reverse) {
      var i = size
      while (isNotEmpty())
        fn(--i, unsafePopLast())
   ...
   */
  fun peekEach(reverse: Boolean = false, fn: (value: Short) -> Unit) {
    if (reverse)
   ...
   * @param fn Function to call with each item in this deque.
   */
  fun peekEachIndexed(reverse: Boolean, fn: (index: Int, value: Short) -> Unit) {
    if (reverse)
      for (i in lastIndex downTo 0)
        fn(i, get(i))
    else
  ...
  fun contentEquals(other: ShortDeque): Boolean {
    if (size != other.size)
      return false
    if (size == 0)
      return true
  ...
  protected fun copyElements(newCapacity: Int) {
    val new = ShortArray(newCapacity)
