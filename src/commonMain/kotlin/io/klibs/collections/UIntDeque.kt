...
open class UIntDeque(
  initialCapacity: Int = 16,
  val scaleFactor: Float = 1.5F,
  val maxSize: Int = 2_147_483_647,
) {

  protected var buffer = UIntArray(initialCapacity)
   ...
   */
  fun popFirstOr(value: UInt) = if (isEmpty()) value else unsafePopFirst()
   ...
   * was empty.
   */
  fun popFirstOrGet(fn: () -> UInt) = if (isEmpty()) fn() else unsafePopFirst()
  ...
  private fun unsafePopFirst(): UInt {
   ...
   */
  fun peekFirstOr(value: UInt) = if (isEmpty()) value else buffer[realHead]
   ...
   */
  fun peekFirstOrGet(fn: () -> UInt) = if (isEmpty()) fn() else buffer[realHead]
   ...
   * if this deque was non-empty, or [value] if this deque was empty.
   */
  fun popLastOr(value: UInt) = if (isEmpty()) value else unsafePopLast()
   ...
   * was empty.
   */
  fun popLastOrGet(fn: () -> UInt) = if (isEmpty()) fn() else unsafePopLast()
  ...
  private fun unsafePopLast(): UInt {
   ...
   * [value] if this deque is empty.
   */
  fun peekLastOr(value: UInt) = if (isEmpty()) value else buffer[internalIndex(lastIndex)]
   ...
   * the value returned by [fn] if this deque is empty.
   */
  fun peekLastOrGet(fn: () -> UInt) = if (isEmpty()) fn() else buffer[internalIndex(lastIndex)]
  ...
  inline fun prepend(value: UInt) = pushFirst(value)
   ...
   */
  @Suppress("NOTHING_TO_INLINE")
  inline fun append(value: UInt) = pushLast(value)
  ...
  @Suppress("NOTHING_TO_INLINE")
  inline operator fun plusAssign(value: UInt) = pushLast(value)
  ...
  fun contentEquals(other: UIntDeque): Boolean {
    if (size != other.size)
      return false
    if (size == 0)
      return true
