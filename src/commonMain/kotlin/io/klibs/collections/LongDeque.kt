...
open class LongDeque(
  initialCapacity: Int = 16,
  val scaleFactor: Float = 1.5F,
  val maxSize: Int = 2_147_483_647,
) {

  protected var buffer = LongArray(initialCapacity)
   ...
   */
  fun popFirstOr(value: Long) = if (isEmpty()) value else unsafePopFirst()
   ...
   * was empty.
   */
  fun popFirstOrGet(fn: () -> Long) = if (isEmpty()) fn() else unsafePopFirst()
  ...
  private fun unsafePopFirst(): Long {
   ...
   */
  fun peekFirstOr(value: Long) = if (isEmpty()) value else buffer[realHead]
   ...
   */
  fun peekFirstOrGet(fn: () -> Long) = if (isEmpty()) fn() else buffer[realHead]
   ...
   * if this deque was non-empty, or [value] if this deque was empty.
   */
  fun popLastOr(value: Long) = if (isEmpty()) value else unsafePopLast()
   ...
   * was empty.
   */
  fun popLastOrGet(fn: () -> Long) = if (isEmpty()) fn() else unsafePopLast()
  ...
  private fun unsafePopLast(): Long {
   ...
   * [value] if this deque is empty.
   */
  fun peekLastOr(value: Long) = if (isEmpty()) value else buffer[internalIndex(lastIndex)]
   ...
   * the value returned by [fn] if this deque is empty.
   */
  fun peekLastOrGet(fn: () -> Long) = if (isEmpty()) fn() else buffer[internalIndex(lastIndex)]
  ...
  inline fun prepend(value: Long) = pushFirst(value)
   ...
   */
  @Suppress("NOTHING_TO_INLINE")
  inline fun append(value: Long) = pushLast(value)
  ...
  @Suppress("NOTHING_TO_INLINE")
  inline operator fun plusAssign(value: Long) = pushLast(value)
   ...
   */
  fun popEach(reverse: Boolean = false, fn: (value: Long) -> Unit) {
    if (reverse)
      while (isNotEmpty())
        fn(unsafePopLast())
    else
   ...
   * @param fn Function to call with each item removed from this deque.
   */
  fun popEachIndexed(reverse: Boolean = false, fn: (index: Int, value: Long) -> Unit) {
    if (reverse) {
      var i = size
      while (isNotEmpty())
        fn(--i, unsafePopLast())
   ...
   */
  fun peekEach(reverse: Boolean = false, fn: (value: Long) -> Unit) {
    if (reverse)
   ...
   * @param fn Function to call with each item in this deque.
   */
  fun peekEachIndexed(reverse: Boolean, fn: (index: Int, value: Long) -> Unit) {
    if (reverse)
      for (i in lastIndex downTo 0)
        fn(i, get(i))
    else
  ...
  fun contentEquals(other: LongDeque): Boolean {
    if (size != other.size)
      return false
    if (size == 0)
      return true
